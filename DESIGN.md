...
`ProbBars` と `DivergingBars` が共有する目盛り。`1fr 1fr 1fr` のグリッドで
...
`DIVERGING_SCALE` を変えるときは、そちらの文も必ず直すこと。
...
- **軸の下に実レンジと幅を必ず併記する。** これが無いと点の位置だけでは
  差の大きさが分からない。ここだけ `ScaleAxis` を使わず、同じ形の目盛りを
  自前で描いている（左右の余白は 30px / 70px。値ラベルが 60px 幅のため）
