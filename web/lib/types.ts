...
export interface Race {
  ...
  picks?: TrifectaPick[];
  /**
   * 三連複の推奨買い目。EVの高い順。三連単より本数が少ない（的が20通りしか
   * ないため）。三連複のオッズが取れていないレースでは付かない。三連単だけが
...
/**
 * 買い目1点。三連単（picks）と三連複（trio_picks）で共通の形。
 * 券種の違いは combo の読み方に出る。
 */
export interface TrifectaPick {
  /**
   * "1-3-5" の形。三連単では 1着-2着-3着の順、
   * 三連複では着順を問わない3艇を昇順に並べたもの。
   */
  combo: string;
  /** モデルがこの目に与えた確率。 */
  prob: number;
  /** 表示時点のオッズ。券種ごとに別の市場の値。 */
  odds: number;
